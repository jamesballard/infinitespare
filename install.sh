...
install_infiniterooms() {
	...
	ln -snf /var/infiniterooms/$stage /var/www/$stage/infiniterooms/tmp
	ln -snf /var/log/infiniterooms/$stage /var/www/$stage/infiniterooms/tmp/logs
...
EOF
	...
	if $dbnew; then
		cd /var/www/$stage/infiniterooms/
		cat Config/Schema/{infinitecake.sql,memberships.sql,members.sql,aros.sql} | grep -v "^USE" | sed -e 's/DEFINER=`root`@`localhost`//' | mysql -B $dbname
